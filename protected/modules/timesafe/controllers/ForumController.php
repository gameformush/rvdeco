<?php
class ForumController extends RController
{
    ...
    public function actionList() {
        ...
        if (isset($_GET['Forum'])) {
            ...
            Yii::app()->user->setState('_filter_Forum', $_GET['Forum']);
        } else
            if (Yii::app()->user->hasState('_filter_Forum')) {
                $model->attributes = Yii::app()->user->getState('_filter_Forum');
            }
    ...
    public function actionUpdate($id) {
        ...
        $this->performAjaxValidation($model);
        $page = (int)Yii::app()->request->getParam('Forum_page');
    ...
    public function beforeAction($action) {
        if ($_GET['ajax'] === 'state')
            if (count($_GET['ForumCheck']) > 0) {
                foreach ($_GET['ForumCheck'] as $column => $value) {
                    foreach ($value as $id => $val) {
                        Forum::model()->updateByPk((int)$id, array($column => (int)$val));
                    }
                }
                Yii::app()->end();
            }
