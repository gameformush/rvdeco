...
class CatalogController extends RController
{
    ...
    public function actionList() {
        ...
        if (isset($_GET['Catalog'])) {
            ...
            Yii::app()->user->setState('_filter_Catalog', $_GET['Catalog']);
        } else
            if (Yii::app()->user->hasState('_filter_Catalog')) {
                $model->attributes = Yii::app()->user->getState('_filter_Catalog');
            }
    ...
    public function actionUpdate($id) {
        ...
        $this->performAjaxValidation($model);
        $page = (int)Yii::app()->request->getParam('Catalog_page');
    ...
    public function beforeAction($action) {
        if ($_GET['ajax'] === 'state')
            if (count($_GET['CatalogCheck']) > 0) {
                foreach ($_GET['CatalogCheck'] as $column => $value) {
                    foreach ($value as $id => $val) {
                        Catalog::model()->updateByPk((int)$id, array($column => (int)$val));
                    }
                }
                Yii::app()->end();
            }
